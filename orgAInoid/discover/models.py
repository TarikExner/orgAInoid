...
def sn_conv_t(c_in, c_out, k=4, s=2, p=1, op=0):
    return nn.utils.spectral_norm(
        nn.ConvTranspose2d(c_in, c_out, k, s, p, output_padding=op, bias=False)
    )
...
class Encoder(nn.Module):
    def __init__(self, in_channels=3, latent_dim=350, base_channels=128, img_size=224):
        ...
        self.stem = nn.Sequential(
            sn_conv(in_channels, c, k=3, s=1, p=1), nn.ReLU(inplace=True)
        )
...
def build_models(
    *,
    ...
    Encoder, Decoder, Union[PatchGANDiscriminator, MLPDiscriminator], Disentangler
]:
