...
def score_classifier(true_arr: np.ndarray, pred_arr: np.ndarray, readout: str):
    if true_arr.shape[0] != 0:
        ...
        if readout in ["RPE_Final", "Lens_Final"]:
            ...
            conf_matrix = confusion_matrix(
                true_arr, pred_arr, labels=[0, 1, 2, 3]
            ).ravel()
...
def write_to_scores(string_to_add, output_dir: str, key: str, init: bool = False):
    if init:
