...
def instantiate_model(
    ...
    model = _instantiate_model(
        model=model, eval_set=eval_set, val_exp=val_exp, readout=readout
    )
...
def _create_dataloader(dataset, readout):
    return create_dataloader(
        dataset.X, dataset.y[readout], batch_size=128, shuffle=False, train=False
    )
...
def create_weights(val_scores, method="f1", normalize=True, power=1.0, temperature=1.0):
    ...
    if method == "f1":
        ...
        raise ValueError(
            f"Unknown method '{method}'. Available methods are 'f1', 'inverse', 'softmax', 'power', 'temperature'."
        )
    ...
    weights = {
        model_name: weight for model_name, weight in zip(model_names, weights_array)
    }
...
def neural_net_evaluation(
    cross_val_experiments: list[str],
    ...
    if eval_set == "test":
        ...
        val_loader = create_dataloader(
            X_test, y_test, batch_size=128, shuffle=False, train=False
        )
    else:
    ...
    conf_matrix = confusion_matrix(
        df["truth"].to_numpy(), df["pred"].to_numpy(), labels=labels
    )
    ...
    neural_net_f1 = neural_net_f1.reset_index().melt(
        id_vars="loop", value_name="F1", var_name="Neural Net"
    )

    return neural_net_f1, conf_matrix, confusion_matrices


def _get_unique_experiment_well_combo(df: pd.DataFrame, col1: str, col2: str):
...
def _calculate_train_test_split(df):
    unique_well_per_experiment = _get_unique_experiment_well_combo(
        df, "experiment", "well"
    )
    train_wells, test_wells = train_test_split(
        unique_well_per_experiment, test_size=0.1, random_state=187
    )
...
def _assemble_morphometrics_dataframe(
    ...
    if eval_set == "val":
        ...
        y_train = _one_hot_encode_labels(X_train[readout].to_numpy(), readout=readout)
        train_truth = pd.DataFrame(
            data=np.argmax(y_train, axis=1), columns=["truth"], index=X_train.index
        )
        ...
        X_val = val_df.copy()
        y_val = _one_hot_encode_labels(X_val[readout].to_numpy(), readout=readout)
        val_truth = pd.DataFrame(
            data=np.argmax(y_val, axis=1), columns=["truth"], index=val_df.index
        )
        ...
        X_test = test_df
        y_test = _one_hot_encode_labels(X_test[readout].to_numpy(), readout=readout)
        test_truth = pd.DataFrame(
            data=np.argmax(y_test, axis=1), columns=["truth"], index=X_test.index
        )
        ...
        y_val = _one_hot_encode_labels(X_val[readout].to_numpy(), readout=readout)
        val_truth = pd.DataFrame(
            data=np.argmax(y_val, axis=1), columns=["truth"], index=X_val.index
        )
...
def _get_best_params(classifier: str, readout: str) -> dict:
    with open(
        f"../../shape_analysis/results/best_params/best_params_{classifier}_{readout}.dict",
        "rb",
    ) as file:
...
def _get_classifier(readout):
    if readout == "RPE_Final":
        ...
        return MultiOutputClassifier(
            QuadraticDiscriminantAnalysis(**best_params), n_jobs=16
        )
    elif readout == "RPE_classes":
        best_params = _get_best_params("HistGradientBoostingClassifier", readout)
        return MultiOutputClassifier(
            HistGradientBoostingClassifier(**best_params), n_jobs=16
        )
...
def classifier_evaluation(
    train_experiments, val_experiment_id, readout, eval_set: Literal["test", "val"]
):
    labels = [0, 1] if readout in ["RPE_Final", "Lens_Final"] else [0, 1, 2, 3]
    X_train, y_train, X_test, y_test, X_val, y_val, data_columns = (
        _assemble_morphometrics_dataframe(
            train_experiments, val_experiment_id, readout, eval_set
        )
    )
    ...
    if eval_set == "val":
        pred_values = pd.DataFrame(
            data=np.argmax(clf.predict(X_val[data_columns]), axis=1),
            columns=["pred"],
            index=X_val.index,
        )
        ...
        pred_values = pd.DataFrame(
            data=np.argmax(clf.predict(X_test[data_columns]), axis=1),
            columns=["pred"],
            index=X_test.index,
        )
    ...
    result = result.sort_values(
        ["experiment", "well", "loop", "slice"], ascending=[True, True, True, True]
    )

    f1_scores = calculate_f1_scores(result)
    conf_matrix = confusion_matrix(
        result["truth"].to_numpy(), result["pred"].to_numpy(), labels=labels
    )
