...
def freezing_test_3(
    ...
    In contrast to the first freezing test, we freeze everything but the last
    3 layers of the respective models before training. This will skip the
    initial pass through an unfrozen model and hopefully enable some more training
    capacity.

    Another important change is the change to torch.argmax for the predictions.

    No re-read of the model if the validation metric doesnt improve
    """
    ...
    for epoch in range(n_epochs):
        ...
        if test_loss < best_test_loss:
            ...
            torch.save(
                model.state_dict(),
                os.path.join(
                    model_output_dir, f"{model.__class__.__name__}_{readout}.pth"
                ),
            )
...
def hyperparameter_model_search(
    experiment_id: str,
    dataset_id: str,
    models: list,
    output_dir="./results",
    ...
    for model_class in models:
        ...
        for lr in learning_rates:
            for batch_size in batch_sizes:
                # Print current configuration
                print(
                    f"[INFO] Starting experiment with Model: {model_class.__name__}, "
                ...
                for epoch in range(num_epochs):
                    ...
                    with torch.no_grad():
                        for data, target in val_loader:
                            ...
                            val_loss += loss.item()
                            val_preds += (
                                torch.round(torch.sigmoid(output)).cpu().tolist()
                            )
                            val_true += target.cpu().tolist()
                    ...
                    if new_lr < current_lr:
                        print(
                            f"[INFO] Learning rate reduced from {current_lr} to {new_lr}"
                        )
