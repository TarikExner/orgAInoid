    ...
    if readout == "RPE_Final":
        for classifier in CLASSIFIERS_TO_TEST_RPE:
            _run_hyperparameter_tuning(
                df, output_dir, classifier, data_columns, readout, analysis_id
            )
...
def _run_hyperparameter_tuning(
    ...
    try:
        if classifier in already_calculated[readout]:
            print(
                f"Skipping {classifier}, as it has already been calculated for readout {readout}"
            )
            return
    except KeyError:
        pass
    ...
    for readout in readouts:
        ...
        if not os.path.isfile(param_file_dir):
            ...
            clf = _get_classifier(classifier_name=classifier, hyperparameter=True)
            pipe = Pipeline(
                [
