...
def convexity(regionprops_table: dict):
    ...
    hull_perimeter = np.sum(np.sqrt(np.sum(np.diff(coords[hull.vertices], axis=0) ** 2, axis=1)))
    hull_perimeter += np.sqrt(np.sum((coords[hull.vertices[0]] - coords[hull.vertices[-1]]) ** 2))
    perimeter = regionprops_table["perimeter"][0]
