...
def run_morphometrics(experiment_id: str,
                      ...
                      analysis_id: Optional[str] = None) -> pd.DataFrame:
    """\
    Convention:
    ...
    for i, row in enumerate(individual_organoids.iterrows()):
        if i%10 and i != 0:
            stop = time.time()
            print(f"Processed {i+1}/{individual_organoids.shape[0]+1} loops in {round(stop-start,2)} seconds")
            start = time.time()
    ...
    if not analysis_id:
        output_file_name = os.path.join(output_dir, f"{experiment_id}_morphometrics.csv")
    else:
        output_file_name = os.path.join(output_dir, f"{experiment_id}_morphometrics_{analysis_id}.csv")
