...
def _run_z_projection(
    imgs: list[OrganoidImage], projection: Optional[Literal["sum", "max"]]
) -> OrganoidImage:
    if not projection:
...
def _calculate_morphometrics_on_image(
    original_image: OrganoidImage,
    ...
    reg_table_df = reg_table_df.drop(
        ["coords", "image_convex", "image_filled", "image_intensity"], axis=1
    )
...
def run_morphometrics(
    ...
    z_projection: Optional[Literal["max", "sum"]] = None,
    ...
    if os.path.isfile(overview_file):
        ...
        annotations_file = os.path.join(
            metadata_dir, f"{experiment_id}_annotations.csv"
        )
    ...
    if z_projection:
        constant_columns = [
            col for col in result_dataframe.columns if col not in ["file_name", "slice"]
        ]
        ...
        assert result_dataframe.shape[0] == individual_organoids.shape[0], (
            result_dataframe.shape[0],
            individual_organoids.shape[0],
        )
    ...
    for i, row in enumerate(individual_organoids.iterrows()):
        if i % 10 and i != 0:
            stop = time.time()
            print(
                f"Processed {i + 1}/{individual_organoids.shape[0] + 1} loops in {round(stop - start, 2)} seconds"
            )
            start = time.time()
        ...
        if z_projection is not None:
            ...
            imgs = [OrganoidImage(image_path) for image_path in image_paths]
    ...
    if not analysis_id:
        output_file_name = os.path.join(
            output_dir, f"{experiment_id}_morphometrics.csv"
        )
    else:
        output_file_name = os.path.join(
            output_dir, f"{experiment_id}_morphometrics_{analysis_id}.csv"
        )

    result_dataframe.to_csv(output_file_name, index=False)
