...
def save_saliency_h5(
    all_results: dict, experiment: str, well: str, readout: str, output_dir: str
):
        ...
        for well_key, loops in all_results.items():
            ...
            for loop_idx, sample in loops.items():
                ...
                grp_loop.create_dataset(
                    "input_image", data=img, compression="gzip", chunks=img.shape
                )

                msk = sample["mask"].astype(np.float32)
                grp_loop.create_dataset(
                    "mask", data=msk, compression="gzip", chunks=msk.shape
                )
                ...
                for model_name, algs in sample.items():
                    ...
                    for fn_name, statuses in algs.items():
                        ...
                        grp_fn.create_dataset(
                            "trained", data=tr, compression="gzip", chunks=tr.shape
                        )
                        bl = statuses["baseline"].astype(np.float32)
                        grp_fn.create_dataset(
                            "baseline", data=bl, compression="gzip", chunks=bl.shape
                        )

    print(f"Saved saliency maps to {path}")


def compute_saliencies(
    ...
    models = initialize_models(
        cnn_models, experiment, readout, model_directory, baseline_directory
    )

    target_layers = {model: _define_target_layers(models[model]) for model in models}

    all_results = {}

    for well in organoid_wells:
        loops = (
            dataset.metadata.loc[dataset.metadata["well"] == well, "loop"]
            .sort_values()
            .tolist()
        )
        ...
        for i, ((img, _cls), mask, loop) in enumerate(
            tqdm(
                zip(cnn_loader, masks, loops),
                desc=f"Well {well}",
                total=len(cnn_loader),
            )
        ):
            image = images[i][0]
            ...
            _baseline = create_baseline_image(img, mask.unsqueeze(0), method="mean").to(
                DEVICE
            )
