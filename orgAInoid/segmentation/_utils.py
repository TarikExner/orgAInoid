...
def train_transformations(image_size):
    return A.Compose(
        [
            ...
            A.RandomResizedCrop(
                height=image_size, width=image_size, scale=(0.8, 1.0), p=0.5
            ),
...
def _run_segmentation_train_loop(
    dataset_dir: str,
    image_size: int,
    batch_size: int,
    ...
    score_file = os.path.join(
        score_output_dir, f"losses_{model.__class__.__name__}_{image_size}.txt"
    )
    ...
    for epoch in range(n_epochs):
        ...
        with open(score_file, "a") as f:
            f.write(
                f"{model.__class__.__name__},{epoch + 1},{train_loss},{val_loss},{batch_size}\n"
            )
...
def create_dataloaders(
    ...
    image_size: float,
    batch_size: int,
    model_name: Literal["UNet", "DEEPLABV3", "HRNET"],
) -> tuple[DataLoader, ...]:
    device = "cuda" if torch.cuda.is_available() else "cpu"
    pin_memory = True if device == "cuda" else False

    imgs = np.load(
        os.path.join(dataset_dir, f"unet_segmentation_images_{image_size}.npy")
    )
    masks = np.load(
        os.path.join(dataset_dir, f"unet_segmentation_masks_{image_size}.npy")
    )
...
def _apply_train_test_split(
    imgs: list[str], masks: list[str], test_split_ratio=0.1
) -> tuple[np.ndarray, ...]:
    split = train_test_split(imgs, masks, test_size=test_split_ratio, random_state=187)
