...
def _assemble_file_paths(input_dir: str) -> tuple[list, list]:
    ...
    imgs = [
        file
        for file in files
        if "_mask" not in file and file.endswith(".tif") and not file.startswith(".")
    ]
    masks = [
        file
        for file in files
        if "_mask" in file and file.endswith(".tif") and not file.startswith(".")
    ]
...
def assemble_data(target_size: int, input_dir: str, output_dir: str = "./raw_data"):
    ...
    np.save(
        os.path.join(output_dir, f"unet_segmentation_images_{target_size}.npy"),
        img_array,
    )
    np.save(
        os.path.join(output_dir, f"unet_segmentation_masks_{target_size}.npy"),
        mask_array,
    )
