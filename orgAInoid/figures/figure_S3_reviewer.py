...
def _generate_main_figure(rpe_final_res: pd.DataFrame,
    ...
    def generate_subfigure_a(fig: Figure,
        ...
        sns.boxplot(**plot_params,
                    ...
                    linewidth = 1,
                    dodge = False)
        clf_comp_plot.set_title(f"Classifier performance for RPE emergence\non image projection {proj}", fontsize = cfg.TITLE_SIZE)
    ...
    def generate_subfigure_b(fig: Figure,
        ...
        sns.boxplot(**plot_params,
                    ...
                    linewidth = 1,
                    dodge = False)
        clf_comp_plot.set_title(f"Classifier performance for lens emergence\non image projection {proj}", fontsize = cfg.TITLE_SIZE)
        clf_comp_plot.set_ylabel("F1 Score", fontsize = cfg.AXIS_LABEL_SIZE)
        clf_comp_plot.set_xticklabels(clf_comp_plot.get_xticklabels(), ha = "right", rotation = 45, fontsize = cfg.AXIS_LABEL_SIZE)
        clf_comp_plot.set_xlabel("", fontsize = cfg.AXIS_LABEL_SIZE)
        clf_comp_plot.set_ylim(-0.03, 1.03)
...
def figure_S3_reviewer_generation(classifier_results_dir: str,
                                  figure_data_dir: str,
                                  figure_output_dir: str,
                                  **kwargs) -> None:
    ...
    lens_final_clf = get_classifier_comparison(classifier_results_dir = classifier_results_dir,
                                               readout = "Lens_Final",
                                               proj = "ZSUM",
                                               output_dir = figure_data_dir)
