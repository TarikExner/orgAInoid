...
Readouts = Literal[
    "RPE_Final", "Lens_Final", "RPE_classes", "Lens_classes", "morph_classes"
]
...
METADATA_COLUMNS = [
    ...
    "RPE_Norin",
    "RPE_Cassian",
    "Confidence_score_RPE",
    "Total_RPE_amount",
    "RPE_classes",
    "Lens_Final",
    "Lens_Norin",
    "Lens_Cassian",
    "Confidence_score_lens",
    "Lens_area",
    "Lens_classes",
    "label",
    "morph_classes",
...
PROJECTION_SAVE_MAP: dict[
    Union[Projections, ProjectionIDs], Union[Projections, ProjectionIDs]
] = {
    "SL3": "SL3",
    "ZMAX": "max",
    "ZSUM": "sum",
...
_CONTAINS_MAP = {"RPE_Final_Contains": "RPE_Final", "Lens_Final_Contains": "Lens_Final"}
_STRING_LABEL_COLS = {"RPE_Final", "Lens_Final"}

_BINARY_MAP = {"yes": 1, "no": 0}


def get_morphometrics_frame(
    results_dir: str,
    ...
    morph_classes = pd.read_csv(
        os.path.join(results_dir, f"morph_classes{suffix}.csv"), index_col=False
    )
    df = df.merge(
...
def f1_scores(
    df: pd.DataFrame, group_keys: list[str], average: str = "weighted"
) -> pd.DataFrame:
    """
...
def add_loop_from_timeframe(
    ...
    max_loops = {tf: min(tf * chunk, total_loops) for tf in range(1, n_timeframes + 1)}
    df = df.copy()
...
def read_classification_results(results_dir, readout: Readouts) -> pd.DataFrame:
    scores = pd.read_csv(os.path.join(results_dir, f"{readout}.txt"))
    scores = _preprocess_results_file(scores)
    return scores


def _get_model(model_name: str, num_classes: int) -> torch.nn.Module:
...
def _instantiate_model(
    ...
    state_dict_path = os.path.join(classifier_dir, f"{model_file_name}_base_model.pth")
...
def model_setup_with_temperature(
    model_name,
    eval_set: EvaluationSets,
    val_experiment_id: str,
    readout: Readouts,
...
def generate_neural_net_ensemble(
    val_experiment_id: str,
    ...
    output_file_name: str = "model_ensemble",
...
def ensemble_probability_averaging(
    models, dataloader, weights=None
...
def _read_val_dataset_full(raw_data_dir: str, file_name: str) -> OrganoidDataset:
...
def _read_val_dataset_split(
    raw_data_dir: str, file_name: str, readout: Readouts
) -> OrganoidTrainingDataset:
    ds = _read_val_dataset_full(raw_data_dir, file_name)
    return OrganoidTrainingDataset(ds, readout=readout)


def _create_val_loader_full_dataset(
    val_dataset: OrganoidDataset, readout: Readouts
) -> DataLoader:
...
def create_val_loader(
    val_dataset: Union[OrganoidDataset, OrganoidTrainingDataset],
    readout: Readouts,
    eval_set: EvaluationSets,
) -> Union[DataLoader, NoReturn]:
    if eval_set == "test":
        return _create_val_loader_split(val_dataset)
    elif eval_set == "val":
        return _create_val_loader_full_dataset(val_dataset, readout=readout)
    else:
        raise ValueError(
            f"eval_set has to be one of 'test' and 'val'. Received {eval_set}"
        )
...
def _save_neural_net_results(
    ...
    save_dir = os.path.join(output_dir, f"classification_{readout}")
    os.makedirs(save_dir, exist_ok=True)
    file_name = _create_cnn_filename(
        val_experiment_id=val_experiment_id,
    ...
    f1_scores.to_csv(os.path.join(save_dir, f"{file_name}_CNNF1.csv"), index=False)

    np.save(os.path.join(save_dir, f"{file_name}_CNNCM.npy"), confusion_matrices)
    return


def _read_neural_net_results(
    output_dir: str,
    readout: Union[Readouts, BaselineReadouts],
    val_experiment_id: str,
    val_dataset_id: str,
    ...
    if f1_scores is not None and confusion_matrices is not None:
        print(
            f"Reading NeuralNet results for {val_experiment_id}: {readout} ({eval_set})"
        )
...
def _get_best_params(
    hyperparameter_dir: str, projection: str, classifier_name: str, readout: str
) -> dict:
    if not projection:
        projection = "SLICE3"
    ...
    file_name = os.path.join(
        hyperparameter_dir, projection, f"best_params_{classifier_name}_{readout}.dict"
    )
...
def _instantiate_classifier(clf, readout: Readouts, best_params: dict):
...
def _save_classifier_results(
    ...
    f1_scores.to_csv(os.path.join(save_dir, f"{file_name}_CLFF1.csv"), index=False)

    np.save(os.path.join(save_dir, f"{file_name}_CLFCM.npy"), confusion_matrices)
    return


def _read_classifier_results(
    output_dir: str,
    readout: Union[Readouts, BaselineReadouts],
    val_experiment_id: str,
    val_dataset_id: str,
    eval_set: EvaluationSets,
    proj: Projections,
    external_experiment_id: Optional[str],
) -> tuple:
    proj = PROJECTION_SAVE_MAP[proj]
    save_dir = os.path.join(output_dir, f"classification_{readout}")
    file_name = _create_cnn_filename(
    ...
    if f1_scores is not None and confusion_matrices is not None:
        print(
            f"Reading classifier results for {val_experiment_id}: {readout} ({eval_set})"
        )

    return f1_scores, confusion_matrices


def _postprocess_cnn_frame(
    df: pd.DataFrame, eval_set: EvaluationSets, baseline: bool = False
) -> pd.DataFrame:
    _df = df[df["classifier"] == "Ensemble"].copy()
    classifier_name = (
        f"Ensemble_{eval_set}" if not baseline else f"Baseline_Ensemble_{eval_set}"
    )
...
def __neural_net_evaluation(
    ...
    if eval_set == "test":
        val_dataset = _read_val_dataset_split(
            raw_data_dir=raw_data_dir,
            file_name=val_dataset_filename,
            readout=readout if not baseline else original_readout,
        )
    else:
        val_dataset = _read_val_dataset_full(
            raw_data_dir=raw_data_dir, file_name=val_dataset_filename
        )
    ...
    if (
        truth_array.shape[0] > df.index.shape[0]
        or ensemble_pred.shape[0] > df.index.shape[0]
    ):
    ...
    truth_values = pd.DataFrame(data=truth_array, columns=["truth"], index=df.index)

    pred_values = pd.DataFrame(data=ensemble_pred, columns=["pred"], index=df.index)
...
def _neural_net_evaluation(
    ...
    neural_net_f1 = neural_net_f1.reset_index().melt(
        id_vars="loop", value_name="F1", var_name="classifier"
    )
...
def __classifier_evaluation(
    ...
    morphometrics_frame = get_morphometrics_frame(
        results_dir=morphometrics_dir,
        external_experiment_id=external_experiment_id,
        suffix=suffix,
    )
    ...
    best_params = _get_best_params(
        hyperparameter_dir, readout=readout, projection=proj, classifier_name=clf_name
    )
    clf = _instantiate_classifier(
        clf_,
        readout=readout if not baseline else original_readout,
    ...
    result_df["truth"] = (
        np.argmax(y_val, axis=1) if eval_set == "val" else np.argmax(y_test, axis=1)
    )
...
def _classifier_evaluation(
    ...
    f1_scores["classifier"] = (
        f"Morphometrics_{eval_set}"
        if not baseline
        else f"Baseline_Morphometrics_{eval_set}"
    )
...
def neural_net_evaluation_baseline(
    val_dataset_id: str,
    val_experiment_id: str,
    eval_set: EvaluationSets,
...
def neural_net_evaluation_baseline_raw_data(
    val_dataset_id: str,
    val_experiment_id: str,
    eval_set: EvaluationSets,
    readout: BaselineReadouts,
...
def neural_net_evaluation_raw_data(
    val_dataset_id: str,
    val_experiment_id: str,
    eval_set: EvaluationSets,
    readout: Readouts,
...
def classifier_evaluation_raw_data(
    val_experiment_id: str,
    readout: Readouts,
    eval_set: EvaluationSets,
    morphometrics_dir: str,
...
def classifier_evaluation_baseline(
    val_experiment_id: str,
    readout: BaselineReadouts,
    eval_set: EvaluationSets,
    morphometrics_dir: str,
...
def classifier_evaluation_baseline_raw_data(
    val_experiment_id: str,
    readout: BaselineReadouts,
    eval_set: EvaluationSets,
    morphometrics_dir: str,
...
def calculate_f1_weights(
    ...
    for model in raw_scores["Model"]:
        res[f"{model}_test_{experiment}"] = raw_scores.loc[
            raw_scores["Model"] == model, readout_score
        ].iloc[0]

    return res


def _generate_classification_results_external_experiment(
    ...
    for experiment in experiments:
        for eval_set in ["val"]:
            if not baseline:
                weights = calculate_f1_weights(
                    classification_dir=experiment_dir,
                    readout=readout,
...
def _generate_classification_results_raw_data(
    readout: Union[Readouts, BaselineReadouts],
    output_dir: str,
    proj: Projections,
    hyperparameter_dir: str,
