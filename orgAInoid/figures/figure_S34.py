...
def _generate_main_figure(rpe_sal: pd.DataFrame,
    ...
    def generate_subfigure_a(fig: Figure,
        ...
        vote_counts = (data.groupby(
            ["experiment", "well", "loop", "hours", "model", "region"], as_index=False
        )["voted"].sum().rename(columns={"voted": "vote_count"}))
        ...
        for i, m in enumerate(models[:3]):
            ...
            curves = []
            for t in thresholds:
                per_well = (df_m.assign(hit=(df_m["vote_count"] >= t).astype(int))
                                  .groupby(["experiment", "well", "hours"], as_index=False)
                                  .agg(frac=("hit", lambda x: x.sum() / len(x))))
                avg = (per_well.groupby("hours", as_index=False)["frac"].mean()
            ...
            axm.set_title(f"Fraction of labeled regions\n{readout}\n{m}", fontsize=cfg.TITLE_SIZE)
            ...
            if i == 0:
                axm.set_ylabel("Fraction of regions (per well)", fontsize=cfg.AXIS_LABEL_SIZE)
...
def figure_S34_generation(sketch_dir: str,
                          figure_output_dir: str,
    ...
    _generate_main_figure(rpe_sal = rpe_saliency_results,
                          ...
                          figure_name = "Supplementary_Figure_S34")
