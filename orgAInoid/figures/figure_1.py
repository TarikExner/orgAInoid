...
def generate_subfigure_a(fig: Figure,
    ...
    img = cv2.imread('./sketches/Figure_1.png', cv2.IMREAD_UNCHANGED)
...
def generate_subfigure_b(fig: Figure,
    ...
    n_control_organoids = data[data["Condition"] == "0nM"].groupby("experiment").size().tolist()
    rpe_crosstab.plot(kind='bar', stacked=True, ax = rpe_plot, color = ['white', 'black'], edgecolor = 'black', align = "center")
    ...
    rpe_plot.bar(x_positions, n_control_organoids, widths, align = "center", label='No\nWnt-\ntreat-\nment', color='white', edgecolor='black', hatch='////', alpha=0.7)
    ...
    lens_crosstab.plot(kind = "bar", stacked = True, ax = lens_plot, color = ['white', 'cyan'], edgecolor = 'black')
    ...
    class HandlerDiagonalSplitRectangle(HandlerBase):
        def create_artists(self, legend, orig_handle,
            ...
            coords_left = [(xdescent, ydescent),
                           (xdescent + width, ydescent),
                           (xdescent, ydescent + height)]
    
            coords_right = [(xdescent + width, ydescent),
                            (xdescent + width, ydescent + height),
                            (xdescent, ydescent + height)]
    
            triangle_left = mpatches.Polygon(coords_left,
                                             facecolor='black', edgecolor='none',
                                             linewidth=0, transform=trans)
            triangle_right = mpatches.Polygon(coords_right,
                                              facecolor='cyan', edgecolor='none',
                                              linewidth=0, transform=trans)
    
            rect_outline = mpatches.Rectangle((xdescent, ydescent), width, height,
    ...
    custom_handle = mpatches.Rectangle((0, 0), 1, 1, facecolor='none', edgecolor='black', linewidth = 1)
    ...
    lens_plot.legend([handles[0], custom_handle, hatched_handle],
                     labels,
                     handler_map={custom_handle: HandlerDiagonalSplitRectangle()},
                     bbox_to_anchor = (1.01, 0.5), loc = "center left", title = "contains\ntissue", fontsize = cfg.TITLE_SIZE, title_fontsize = cfg.TITLE_SIZE)
...
def generate_subfigure_c(fig: Figure,
    ...
    data["Total_RPE_amount"] = data["Total_RPE_amount"] * cfg.RPE_UM_CONVERSION_FACTOR
    ...
    for cutoff in cfg.RPE_CUTOFFS:
        left_histogram_plot.axvline(x = cutoff * cfg.RPE_UM_CONVERSION_FACTOR, color = "black")
    
    right_histogram_plot.tick_params(**cfg.TICKPARAMS_PARAMS)
    right_histogram_plot.set_title("Lens size distribution", fontsize = cfg.TITLE_SIZE)
...
if __name__ == "__main__":
    ...
    plt.savefig("./prefinal_figures/Figure1.pdf", dpi = 300, bbox_inches = "tight")
    plt.savefig("./prefinal_figures/Figure1.png", dpi = 300, bbox_inches = "tight")
    plt.show()
