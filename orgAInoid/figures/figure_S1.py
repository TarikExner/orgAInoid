...
def generate_subfigure_a(fig: Figure,
    ...
    data = pd.read_csv("./figure_data/dataset_annotations.csv")
    ...
    grouped_data = [area_df[area_df['Condition'] == condition]['RPE_area'] for condition in conditions if condition != "0nM"]
    assert len(grouped_data) == 3
    _, p_value_area = f_oneway(*grouped_data)

    grouped_data = [percentage_df[percentage_df['Condition'] == condition]['RPE_Percentage'] for condition in conditions if condition != "0nM"]
    assert len(grouped_data) == 3
    ...
    rpe_percentage.legend(bbox_to_anchor = (1.01, 0.5), loc = "center left", fontsize = cfg.AXIS_LABEL_SIZE)
    rpe_percentage.set_title("Presence of RPE at last time point\nby Wnt-surrogate", fontsize = cfg.TITLE_SIZE)
    rpe_percentage.set_ylabel("organoids with RPE [%]", fontsize = cfg.AXIS_LABEL_SIZE)
    rpe_percentage.set_xticklabels(["0 nM", "1 nM", "2 nM", "4 nM"])
    rpe_percentage.tick_params(**cfg.TICKPARAMS_PARAMS)
    rpe_percentage.set_xlabel("Wnt-surrogate concentration [nM]", fontsize = cfg.AXIS_LABEL_SIZE)
    rpe_percentage.set_ylim(rpe_percentage.get_ylim()[0], rpe_percentage.get_ylim()[1]*1.2)
    rpe_percentage.text(x = rpe_percentage.get_xlim()[0]+0.1, y = rpe_percentage.get_ylim()[1]*0.8, s = f"ANOVA(Wnt-surrogate > 0nM)\np = {round(p_value_percentage,2)}", fontsize = cfg.AXIS_LABEL_SIZE)
    ...
    rpe_area.legend(bbox_to_anchor = (1.01, 0.5), loc = "center left", fontsize = cfg.AXIS_LABEL_SIZE)
    rpe_area.set_title("RPE amount at last time point\nby Wnt-surrogate", fontsize = cfg.TITLE_SIZE)
    rpe_area.set_ylabel("RPE area [µm²]", fontsize = cfg.AXIS_LABEL_SIZE)
    rpe_area.tick_params(**cfg.TICKPARAMS_PARAMS)
    rpe_area.set_xlabel("Wnt-surrogate concentration [nM]", fontsize = cfg.AXIS_LABEL_SIZE)
    rpe_area.set_xticklabels(["0 nM", "1 nM", "2 nM", "4 nM"])
    rpe_area.set_ylim(rpe_area.get_ylim()[0], rpe_area.get_ylim()[1]*1.2)
    rpe_area.text(x = rpe_area.get_xlim()[0]+0.1, y = rpe_area.get_ylim()[1]*0.8, s = f"ANOVA(Wnt-surrogate > 0nM)\np = {round(p_value_area,2)}", fontsize = cfg.AXIS_LABEL_SIZE)
...
def generate_subfigure_b(fig: Figure,
    ...
    grouped_data = [area_df[area_df['Condition'] == condition]['Lens_area'] for condition in conditions]
    _, p_value_area = f_oneway(*grouped_data)

    grouped_data = [percentage_df[percentage_df['Condition'] == condition]['Lens_Percentage'] for condition in conditions]
    _, p_value_percentage = f_oneway(*grouped_data)
    ...
    lens_percentage.legend(handles, labels, bbox_to_anchor = (1.01, 0.5), loc = "center left", fontsize = cfg.AXIS_LABEL_SIZE)
    lens_percentage.set_title("Presence of lenses at last time point\nby Wnt-surrogate concentration", fontsize = cfg.TITLE_SIZE)
    lens_percentage.set_ylabel("organoids with lens [%]", fontsize = cfg.AXIS_LABEL_SIZE)
    lens_percentage.tick_params(**cfg.TICKPARAMS_PARAMS)
    lens_percentage.set_xlabel("Wnt-surrogate concentration [nM]", fontsize = cfg.AXIS_LABEL_SIZE)
    lens_percentage.set_xticklabels(["0 nM", "1 nM", "2 nM", "4 nM"])
    lens_percentage.set_ylim(lens_percentage.get_ylim()[0], lens_percentage.get_ylim()[1]*1.1)
    lens_percentage.text(x = lens_percentage.get_xlim()[0]+0.1, y = lens_percentage.get_ylim()[1]*0.9, s = f"ANOVA p = {round(p_value_percentage,2)}", fontsize = cfg.AXIS_LABEL_SIZE)
    ...
    lens_area.legend(handles, labels, bbox_to_anchor = (1.01, 0.5), loc = "center left", fontsize = cfg.AXIS_LABEL_SIZE)
    lens_area.set_title("Lens sizes at last time point\nby Wnt-surrogate", fontsize = cfg.TITLE_SIZE)
    lens_area.set_ylabel("lens area [µm²]", fontsize = cfg.AXIS_LABEL_SIZE)
    lens_area.tick_params(**cfg.TICKPARAMS_PARAMS)
    lens_area.set_xlabel("Wnt-surrogate concentration [nM]", fontsize = cfg.AXIS_LABEL_SIZE)
    lens_area.set_xticklabels(["0 nM", "1 nM", "2 nM", "4 nM"])
    lens_area.set_ylim(lens_area.get_ylim()[0], lens_area.get_ylim()[1]*1.1)
    lens_area.text(x = lens_area.get_xlim()[0]+0.1, y = lens_area.get_ylim()[1]*0.9, s = f"ANOVA p = {round(p_value_area,2)}", fontsize = cfg.AXIS_LABEL_SIZE)
...
def generate_subfigure_c(fig: Figure,
    ...
    ax.axis("off")
    utils._figure_label(ax, subfigure_label, x = -0.25)
    data = pd.read_csv("./figure_data/RPE_visibility.csv")
    data["hours"] = data["loop"] / 2
    data = data[data["evaluator"] != "mean"]
    ...
    conf_matrix = np.load("./figure_data/RPE_visibility_conf_matrix.npy")
    ...
    sns.heatmap(conf_matrix, annot = True, fmt = ".0f", ax = conf_matrix_plot, cmap = "Reds", cbar = False, annot_kws={"fontsize": cfg.TITLE_SIZE})
    ...
    conf_matrix_plot.set_title("Confusion matrix\nfor last timeframe", fontsize = cfg.TITLE_SIZE)
...
def generate_subfigure_d(fig: Figure,
    ...
    ax.axis("off")
    utils._figure_label(ax, subfigure_label, x = -0.25)
    data = pd.read_csv("./figure_data/Lens_visibility.csv")
    data["hours"] = data["loop"] / 2
    data = data[data["evaluator"].isin(["HEAT1", "HEAT7"])]
...
def generate_subfigure_e_and_f(fig: Figure,
    ...
    handles = handles[::-1]
    labels = labels[::-1]
    rpe_plot.legend(handles, labels, bbox_to_anchor = (1.01, 0.5), loc = "center left", fontsize = cfg.AXIS_LABEL_SIZE, title = "class", title_fontsize = cfg.AXIS_LABEL_SIZE)
    ...
    handles = handles[::-1]
    labels = labels[::-1]
    lens_plot.legend(handles, labels, bbox_to_anchor = (1.01, 0.5), loc = "center left", fontsize = cfg.AXIS_LABEL_SIZE, title = "class", title_fontsize = cfg.AXIS_LABEL_SIZE)
    return
...
if __name__ == "__main__":
    ...
    plt.savefig("./prefinal_figures/FigureS1.pdf", dpi = 300, bbox_inches = "tight")
    plt.savefig("./prefinal_figures/FigureS1.png", dpi = 300, bbox_inches = "tight")
    plt.show()
