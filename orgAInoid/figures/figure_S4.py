...
def _generate_main_figure(
    rpe_final_res: pd.DataFrame,
    lens_final_res: pd.DataFrame,
    ...
    def generate_subfigure_a(
        ...
        order = (
            data[(data["score_on"] == "val") & (data["readout"] == readout)]
            .groupby("ALGORITHM")
            .median("f1_score")
        ...
        clf_comp_plot.set_title(
            "Classifier performance for RPE emergence", fontsize=cfg.TITLE_SIZE
        )
        clf_comp_plot.set_ylabel("F1 Score", fontsize=cfg.AXIS_LABEL_SIZE)
        clf_comp_plot.set_xticklabels(
            clf_comp_plot.get_xticklabels(),
            ha="right",
            rotation=45,
        ...
        clf_comp_plot.legend(
            bbox_to_anchor=(1.01, 0.37), loc="center left", fontsize=cfg.TITLE_SIZE
        )
    ...
    def generate_subfigure_b(
        ...
        clf_comp_plot.set_title(
            "Classifier performance for lens emergence", fontsize=cfg.TITLE_SIZE
        )
        clf_comp_plot.set_ylabel("F1 Score", fontsize=cfg.AXIS_LABEL_SIZE)
        clf_comp_plot.set_xticklabels(
            clf_comp_plot.get_xticklabels(),
    ...
    gs = GridSpec(ncols=6, nrows=2, figure=fig, height_ratios=[1, 1])
    a_coords = gs[0, :]
    b_coords = gs[1, :]
...
def figure_S4_generation(
    classifier_results_dir: str, figure_data_dir: str, figure_output_dir: str, **kwargs
) -> None:
    rpe_final_clf = get_classifier_comparison(
        classifier_results_dir=classifier_results_dir,
        readout="RPE_Final",
        proj="SL3",
        output_dir=figure_data_dir,
    )
    lens_final_clf = get_classifier_comparison(
        classifier_results_dir=classifier_results_dir,
        readout="Lens_Final",
        proj="SL3",
        output_dir=figure_data_dir,
    ...
    _generate_main_figure(
        rpe_final_res=rpe_final_clf,
        lens_final_res=lens_final_clf,
        figure_output_dir=figure_output_dir,
        figure_name="S4_Fig",
    )

    rpe_output_dir = os.path.join(figure_output_dir, "Extended_Data_15.csv")
    rpe_final_clf.to_csv(rpe_output_dir, index = False)

    lens_output_dir = os.path.join(figure_output_dir, "Extended_Data_16.csv")
