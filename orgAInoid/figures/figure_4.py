...
def _generate_main_figure(
    ...
    def generate_subfigure_c(
        ...
        rpe_attr = _crop_array(
            np.load("./figure_data/relevance_propagation/attributions_RPE_Final.npy")
        )
        lens_attr = _crop_array(
            np.load("./figure_data/relevance_propagation/attributions_Lens_Final.npy")
        )
        rpe_classes_attr = _crop_array(
            np.load("./figure_data/relevance_propagation/attributions_RPE_classes.npy")
        )
        lens_classes_attr = _crop_array(
            np.load("./figure_data/relevance_propagation/attributions_Lens_classes.npy")
        )
        ...
        for loop in loops_to_show:
            ...
            img_plot.set_ylabel(
                f"hours: {int(np.ceil(loop_idxs[loop] / 2))}",
                fontsize=cfg.TITLE_SIZE,
                labelpad=0,
            )

            _add_scalebar(
                img_plot, img.shape, pixel_size_um=pixel_size_um, bar_um=100.0
            )
            ...
            for axis in [
                ...
                rpe_attr_plot,
                lens_attr_plot,
                rpe_classes_attr_plot,
                lens_classes_attr_plot,
            ]:
