...
def _generate_main_figure(
    ...
    def generate_subfigure_a(
        ...
        distance_plot.set_title(
            "Jaccard scores of 30 neighbors in TSNE space vs. 30 neighbors in PCA space (euclidean distance)",
    ...
    def generate_subfigure_b(
        ...
        distance_plot.tick_params(**cfg.TICKPARAMS_PARAMS)
        distance_plot.set_title(
            "Jaccard scores of 30 neighbors in TSNE space vs. 30 neighbors in raw data space (euclidean distance)",
            fontsize=cfg.TITLE_SIZE,
        )
        return
    ...
    def generate_subfigure_c(
        ...
        distance_plot.tick_params(**cfg.TICKPARAMS_PARAMS)
        distance_plot.set_title(
            "Jaccard scores of 30 neighbors in UMAP space vs. 30 neighbors in PCA space (euclidean distance)",
            fontsize=cfg.TITLE_SIZE,
        )
    ...
    def generate_subfigure_e(
        ...
        distance_plot.tick_params(**cfg.TICKPARAMS_PARAMS)
        distance_plot.set_title(
            "Fraction of 30 nearest neighbors (pca space)\nwithin the same organoid",
            fontsize=cfg.TITLE_SIZE,
        )
    ...
    def generate_subfigure_f(
        ...
        distance_plot.tick_params(**cfg.TICKPARAMS_PARAMS)
        distance_plot.set_title(
            "Fraction of 30 nearest neighbors (raw data space)\nwithin the same organoid",
            fontsize=cfg.TITLE_SIZE,
        )
...
def figure_S3_generation(
    ...
    jaccard_output_dir = os.path.join(figure_output_dir, "S13_Data.csv")
