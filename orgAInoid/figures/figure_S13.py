...
def _generate_main_figure(
    ...
    def generate_subfigure_b(
        ...
        for i in range(1, len(test_data.columns)):
            cumulative_base[:, i] = (
                cumulative_base[:, i - 1] + test_data.iloc[:, i - 1].values
            )
        ...
        for i in range(1, len(val_data.columns)):
            cumulative_base[:, i] = (
                cumulative_base[:, i - 1] + val_data.iloc[:, i - 1].values
            )
    ...
    gs = GridSpec(ncols=6, nrows=4, figure=fig, height_ratios=[1, 0.7, 1, 0.7])
    a_coords = gs[0, :]
    b_coords = gs[1, :]
    c_coords = gs[2, :]
    d_coords = gs[3, :]
...
def figure_S13_generation(
    ...
    **kwargs,
    ...
    rpe_cnn_test_cm = create_confusion_matrix_frame(
        readout="RPE_Final",
    ...
    rpe_cnn_val_cm = create_confusion_matrix_frame(
        readout="RPE_Final",
        classifier="neural_net",
        eval_set="val",
        proj="sum",
    ...
    lens_cnn_test_cm = create_confusion_matrix_frame(
        readout="Lens_Final",
        classifier="neural_net",
        eval_set="test",
        proj="sum",
    ...
    lens_cnn_val_cm = create_confusion_matrix_frame(
        readout="Lens_Final",
        classifier="neural_net",
        eval_set="val",
        proj="sum",
    ...
    _generate_main_figure(
        ...
        figure_name="Supplementary_Figure_S13",
    )

    rpe_final_output_dir = os.path.join(figure_output_dir, "Data_S1_SF13a.csv")
    ...
    rpe_cnn_output_dir = os.path.join(figure_output_dir, "Data_S1_SF13b.csv")
    ...
    lens_final_output_dir = os.path.join(figure_output_dir, "Data_S1_SF13c.csv")
    ...
    lens_cnn_output_dir = os.path.join(figure_output_dir, "Data_S1_SF13d.csv")
