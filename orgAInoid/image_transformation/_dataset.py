...
def train_transformations():
    return A.Compose(
        [
            ...
            A.Transpose(p=0.5),
            A.ShiftScaleRotate(
                shift_limit=0.0625, scale_limit=0.1, rotate_limit=45, p=0.75
            ),
            A.ElasticTransform(alpha=1, sigma=50, p=0.5),
            A.RandomResizedCrop(
                height=224, width=224, scale=(0.8, 1.0), ratio=(0.9, 1.1), p=0.5
            ),
...
class ImageSequenceDataset(Dataset):
    def __init__(
        self, image_array, df, num_input=5, num_output=5, gap=0, transform=None
    ):
        """
    ...
    def _prepare_sequences(self):
        ...
        for _, group in grouped:
            ...
            max_start = (
                total_timepoints - self.num_input - self.gap - self.num_output + 1
            )
            for start in range(max_start):
                input_indices = group.loc[
                    start : start + self.num_input - 1, "IMAGE_ARRAY_INDEX"
                ].values
                target_start = start + self.num_input + self.gap
                target_end = target_start + self.num_output
                target_indices = group.loc[
                    target_start : target_end - 1, "IMAGE_ARRAY_INDEX"
                ].values
        ...
        print(
            f"Total sequences available with gap={self.gap}: {len(self.sequence_indices)}"
        )
    ...
    def __getitem__(self, idx):
        ...
        transformed_inputs = transformed_images[
            : self.num_input
        ]  # Shape: (5, 1, 224, 224)
        transformed_targets = transformed_images[
            self.num_input :
        ]  # Shape: (5, 1, 224, 224)
...
def create_dataset(
    ...
    return ImageSequenceDataset(
        img_array, metadata, num_input, num_output, gap, transformations
    )
