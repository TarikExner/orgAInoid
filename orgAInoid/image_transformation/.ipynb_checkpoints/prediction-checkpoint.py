...
def plot_current_state(images: np.ndarray,
                       target: np.ndarray,
                       model,
                       epoch: int,
                       device: str,
    ...
    plt.savefig(os.path.join(image_output_dir, f"{experiment_id}_epoch_{epoch}.pdf"), dpi = 300, bbox_inches = "tight")
    plt.show()
...
class ImageSequenceDataset(Dataset):
    ...
    def __len__(self):
        ...
        return (self.num_timepoints - self.subseries_length - self.prediction_length + 1) * self.num_sequences

    def __getitem__(self, idx):
        sequence_idx = idx // (self.num_timepoints - self.subseries_length - self.prediction_length + 1)
        timepoint_idx = idx % (self.num_timepoints - self.subseries_length - self.prediction_length + 1)
        
        subseries = self.image_sequences[sequence_idx, timepoint_idx:timepoint_idx + self.subseries_length]
        target = self.image_sequences[sequence_idx, timepoint_idx + self.subseries_length : timepoint_idx + self.subseries_length + self.prediction_length]
...
def run_prediction_v2(experiment_id: str,
    ...
    train_data = ImageSequenceDataset(X_train, subseries_length, prediction_length = prediction_length)
    test_data = ImageSequenceDataset(X_test, subseries_length, prediction_length = prediction_length)
    ...
    model = ImageTransformerV2(img_size, patch_size, subseries_length, embed_dim, num_heads, num_layers, dropout, prediction_length=prediction_length)
    if from_state_dict:
    ...
    if mode == "gif_generation":
        torch.save(model.state_dict(), os.path.join(model_output_dir, f"{experiment_id}_epoch_0.pth"))
    ...
    if not os.path.isfile(score_file):
        ...
        with open(score_file, 'w') as f:
            f.write('epoch,train_loss,val_loss,batch_size,img_size,patch_size,num_layers,num_heads,embed_dim,forward_expansion,dropout,subseries_length,prediction_length\n')
    ...
    for epoch in range(num_epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), os.path.join(model_output_dir, f"{experiment_id}_best_model.pth"))

        if mode == "gif_generation":
            torch.save(model.state_dict(), os.path.join(model_output_dir, f"{experiment_id}_epoch_{epoch+1}.pth"))
            

        with open(score_file, 'a') as f:
            img_size,patch_size,num_layers,num_heads,embed_dim,forward_expansion,dropout
            f.write(
...
def run_prediction(experiment_id: str,
                   subseries_length: int = 5,
                   batch_size: int = 64,
                   img_size: tuple[int] = (128,128),
                   patch_size: int = 16,
    ...
    model = ImageTransformer(img_size, patch_size, subseries_length, embed_dim, num_heads, num_layers, dropout)
    device = "cuda" if torch.cuda.is_available() else "cpu"
    model = model.to(device)
    ...
    if not os.path.isfile(score_file):
        print("initializing score file")
        with open(score_file, 'w') as f:
            f.write('epoch,train_loss,val_loss,batch_size,img_size,patch_size,num_layers,num_heads,embed_dim,forward_expansion,dropout\n')
