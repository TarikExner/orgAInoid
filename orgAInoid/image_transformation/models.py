...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(
            batch_size * num_images, in_channels, img_size, img_size
        )  # (B*num_images, C, H, W)
        ...
        x = x.reshape(
            batch_size, num_images * self.num_patches_per_image, -1
...
class VisionTransformerPredictor(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.cat(
            (cls_tokens, x), dim=1
        )  # (B, 1 + num_input_images*num_patches, embed_dim)
        ...
        x = self.pred_head(
            cls_output
        )  # (batch_size, num_predicted_images * img_size * img_size)
        x = x.view(
            batch_size,
            self.num_predicted_images,
            1,
...
class ImageTransformerV2(nn.Module):
    def __init__(
        ...
        self.pos_encoder = nn.Parameter(
            torch.zeros(1, subseries_length * self.num_patches, embed_dim)
        )
        ...
        self.transformer_encoder = nn.TransformerEncoder(
            encoder_layers, num_layers=num_layers, enable_nested_tensor=True
        )
    ...
    def forward(self, x):
        ...
        x = x.unsqueeze(
            2
        )  # Now x.shape: [batch_size, subseries_length, 1, height, width]

        # Flatten the subseries_length into the batch dimension to process patches
        x = x.view(
            -1, 1, height, width
        )  # Flatten subseries_length into the batch dimension: [batch_size * subseries_length, 1, height, width]

        # Apply patch embedding
        x = self.patch_embedding(
            x
        )  # [batch_size * subseries_length, embed_dim, height/patch_size, width/patch_size]
        ...
        x = x.reshape(
            batch_size, subseries_length * self.num_patches, self.embed_dim
        )  # [batch_size, subseries_length * num_patches, embed_dim]

        # Step 2: Positional Encoding
        dynamic_pos_encoder = self.pos_encoder[
            :, : subseries_length * self.num_patches, :
        ]
        x = (
            x + dynamic_pos_encoder
        ...
        x = self.decoder(
            x
        )  # [batch_size, seq_length, patch_size * patch_size * prediction_length]

        # Reshape the output to form the predicted patches for each future image
        x = x.view(
            batch_size, -1, self.patch_size, self.patch_size
        )  # [batch_size, seq_length * prediction_length, patch_size, patch_size]

        # Focus only on the last set of patches for the last image in the predicted sequence
        x = x[
            :, -self.num_patches * self.prediction_length :, :, :
        ]  # [batch_size, num_patches * prediction_length, patch_size, patch_size]
        ...
        x = x.reshape(
            batch_size * self.prediction_length,
            num_patches_per_dim_height,
            num_patches_per_dim_width,
            self.patch_size,
            self.patch_size,
        )
        ...
        x = (
            ...
            .contiguous()
            .view(
                batch_size, self.prediction_length, self.img_size[0], self.img_size[1]
            )
        )
