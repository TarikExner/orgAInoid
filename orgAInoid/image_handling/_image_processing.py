...
class ImageProcessor:
    ...
    def _threshold_mask(
        self, img: np.ndarray, threshold: float, copy: bool = False
    ) -> np.ndarray:
    ...
    def threshold_mask(
        ...
        return self._threshold_mask(img, threshold, copy)

    def _min_max_scale(
        self, img: np.ndarray, mask_array: Optional[np.ndarray] = None
    ) -> np.ndarray:
        ...
        if mask_array is None:
            ...
            img_rescaled[mask_array] = (img_rescaled[mask_array] - min_val) / (
                max_val - min_val
            )
            return img_rescaled

    def preprocess_for_segmentation(
        self, img: np.ndarray, segmentator_input_size: int, copy: bool = True
    ) -> np.ndarray:
    ...
    def _already_has_right_size(self, img: np.ndarray, size: int):
        return img.shape == (size, size)

    def _downsample(self, img: np.ndarray, target_size: int) -> np.ndarray:
    ...
    def downsample(self, img: np.ndarray, target_size: int) -> np.ndarray:
        return self._downsample(img, target_size)

    def _upsample(self, img: np.ndarray, target_size: int) -> np.ndarray:
        if self._already_has_right_size(img, target_size):
            return img
        return cv2.resize(
    ...
    def upsample(self, img: np.ndarray, target_size: int) -> np.ndarray:
        return self._upsample(img, target_size)

    def crop_to_mask_bounding_box(
        self, mask: OrganoidMask, img: OrganoidImage, rescale=True, crop_size=None
    ) -> tuple[OrganoidImage, OrganoidMask]:
        """
        ...
        if not mask.is_binary():
            thresholded_mask = self._threshold_mask(
                mask.image, threshold=0.5, copy=True
            )
        ...
        if rescale:
            ...
            assert crop_size is not None, (
                "crop_size must be provided when flexible is False"
            )
            ...
            img_padded = cv2.copyMakeBorder(img_cropped, **padding_kwargs)
            mask_padded = cv2.copyMakeBorder(mask_cropped, **padding_kwargs)
