...
class ImageHandler:
    ...
    def get_mask_and_image(
        ...
        crop_bounding_box_dimension: Optional[int] = 320,
        ...
        if mask.dimension != image_target_dimension:
            if mask.dimension < image_target_dimension:
                mask.image = self.img_processor.upsample(
                    mask.image, image_target_dimension
                )
            else:
                mask.image = self.img_processor.downsample(
                    mask.image, image_target_dimension
                )
        ...
        if img.dimension != image_target_dimension:
            if img.dimension < image_target_dimension:
                img.image = self.img_processor.upsample(
                    img.image, image_target_dimension
                )
            else:
                img.image = self.img_processor.downsample(
                    img.image, image_target_dimension
                )
    ...
    def _create_mask_from_image(self, img: OrganoidImage) -> OrganoidMask:
        """\
        ...
        preprocessed = self.img_processor.preprocess_for_segmentation(
            img_array, self.segmentator_input_size
        )
    ...
    def _initialize_segmentation_model(
        self,
        model_name: Literal["HRNET", "UNET", "DEEPLABV3"],
        input_dir: str,
        input_size: int,
    ) -> MaskPredictor:
        kwargs = {"input_dir": input_dir, "input_size": input_size}
        if model_name == "UNET":
