...
class MaskPredictor:
    def __init__(
        self, input_dir: str, input_size: int, model: Union[HRNET, DEEPLABV3, UNet]
    ) -> None:
    ...
    def _load_weights(self):
        ...
        self.model.load_state_dict(
            torch.load(state_dict_path, map_location=torch.device(self.device))
        )
        self.model.eval()
        self.model.to(self.device)

    def _predict_mask_from_model(self, img_tensor: torch.Tensor) -> torch.Tensor:
        """\
    ...
    def _to_normalized_tensor(self, img: np.ndarray) -> torch.Tensor:
        if img.ndim == 2:
    ...
    def _add_dim_to_tensor(self, input_tensor):
...
class UNetPredictor(MaskPredictor):
    ...
    def __init__(self, input_dir: str, input_size: int) -> None:
        model = UNet()
        super().__init__(input_dir=input_dir, input_size=input_size, model=model)
